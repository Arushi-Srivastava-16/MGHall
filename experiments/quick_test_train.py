"""
Quick test training script with 1000 samples.

This script trains on a small subset to verify the pipeline works before full training.
"""
...
# Create small test splits
def create_test_splits():
    """Create small test splits with 1000 samples."""
    data_dir = Path(__file__).parent.parent / "data" / "processed" / "splits"
    test_dir = Path(__file__).parent.parent / "data" / "processed" / "test_splits"
    ...
    # Create test splits
    train_count = subsample_file(data_dir / "train.jsonl", test_dir / "train.jsonl", 700)
    val_count = subsample_file(data_dir / "val.jsonl", test_dir / "val.jsonl", 150)
    test_count = subsample_file(data_dir / "test.jsonl", test_dir / "test.jsonl", 150)
    
    print(f"Created test splits: {train_count} train, {val_count} val, {test_count} test")
    return test_dir
...
def main():
    ...
    # Create test splits
    print("\nCreating small test splits...")
    test_dir = create_test_splits()
    ...
    try:
        train_loader, val_loader, test_loader = create_dataloaders(
            ...
            cache_dir=Path(__file__).parent.parent / "data" / "graphs" / "test_cache",
        )
    ...
    try:
        ...
        # Save results
        results_path = checkpoint_dir / "test_training_results.json"
        ...
        print("="*60)
        print("\nNext step: Run full training with all 69K samples:")
        print("  cd experiments && python train_gnn_math.py")
